...
public class Reporter : MonoBehaviour
{
	...
	void DrawInfo()
	{
		...
		GUILayout.Space(size.x);
		GUILayout.Space(size.x);
		GUILayout.Box(searchContent, nonStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2));
		tempRect = GUILayoutUtility.GetLastRect();
		var temFilterText = GUI.TextField(tempRect, gmFilterText, searchStyle);
		if (temFilterText != gmFilterText) {
			gmFilterText = temFilterText;
		}
		if (GUILayout.Button(showSceneContent, nonStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2))) {
			GMFunction();
		}
		GUILayout.Space(size.x);
		GUILayout.Label("简易GM命令");
		GUILayout.Space(size.x);
		GUILayout.Space(size.x);
		GUILayout.FlexibleSpace();
		GUILayout.EndHorizontal();
	...
	void drawToolBar()
	{
		...
		toolBarRect.x = 0f;
		toolBarRect.y = size.y * 2f;
		toolBarRect.width = Screen.width;
		toolBarRect.height = size.y * 2 * 1f;
		...
		GUI.skin = toolbarScrollerSkin;
		Vector2 drag = getDrag();
		if ((drag.x != 0) && (downPos != Vector2.zero) && (downPos.y > Screen.height - size.y * 4f)) {
	...
	public void OnGUIDraw()
	{
		...
		logsRect.x = 0f;
		logsRect.y = size.y * 2 * 2f;
		logsRect.width = Screen.width;
		logsRect.height = Screen.height * 0.75f - size.y * 2 * 2f;
		...
		detailRect.x = 0f;
		detailRect.y = Screen.height - size.y * 3 * 2;
		detailRect.width = Screen.width;
		detailRect.height = size.y  * 3 * 2;
    ...
    private string gmFilterText;

    void GMFunction()
    {
	    if (string.IsNullOrEmpty(gmFilterText))
	    {
		    return;
	    }
	    
	    GMNet();
	    Debug.Log(gmFilterText);
    }
    
    void GMNet()
    {
	    var msg = gmFilterText;
	    if (msg.StartsWith("NetUrl"))
	    {
		    Debug.Log("NetUrl 设置");
		    var ss = msg.Split(' ');
		    if (ss.Length != 2 || string.IsNullOrEmpty(ss[1]))
		    {
			    PlayerPrefs.SetInt("connect_use_editor_url",0);
			    return;
		    }
		    int.TryParse(ss[1],out int serverNum);
		    PlayerPrefs.SetInt("connect_use_editor_url",1); 
		    PlayerPrefs.SetInt("connect_select_server",serverNum);
	    }
    }
}
